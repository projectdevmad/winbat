    ...
    function SearchController(searchService, $location, $rootScope, $http, $scope, $translate, $translatePartialLoader, $routeParams, $timeout, $q) {
		...
		function searchData(){
			...
			if ($scope.sortby!= null && $scope.sortby != ""){
				param_plus +="&sortby="+$scope.sortby;
			}
			
			
			var param = "q="+$scope.searchData.keyword+"&page="+$scope.page +"&size="+$scope.pagesize +  param_plus;
			...
			$scope.promise = searchService.search($scope.url + param).then(function(data) {
				...
				if ( $scope.searchResult.length == data.total){
                    $scope.loadEnd = true;
                }
